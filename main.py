...
def train_fn(critic, gen, loader, dataset, step,
    ...
    for batch_idx, (real, _) in enumerate(loop):
        ...
        if iter_num % 500 == 0:
            print(f"current iteration number is {iter_num}")
            with torch.no_grad():
                ...
                plt.savefig(output_dir + 'step_{}_gen_{}'.format(step, iter_num))
    ...
    for num_epochs in config.PROGRESSIVE_EPOCHS[step:]:
        ...
        for epoch in range(num_epochs):
            ...
            if config.SAVE_MODEL:
                torch.save(checkpoint, checkpoint_dir + 'check_point_epoch_%d.pth' % (iter_epoch))
