...
def train_fn(critic, gen, loader, dataset, step,
             alpha, opt_critic, opt_gen, scaler_gen, scaler_critic, iter_num):
    loop = tqdm(loader)
    ...
    for num_epochs in config.PROGRESSIVE_EPOCHS[step:]:
        ...
        for epoch in range(num_epochs):
            ...
            alpha, iter_num = train_fn(critic, gen, loader, dataset, step,
                             alpha, opt_critic, opt_gen, scaler_gen, scaler_critic, iter_num)
            print(f"the current iteration number is {iter_num}")
            checkpoint = {
            ...
            if config.SAVE_MODEL:
                torch.save(checkpoint, checkpoint_dir + 'checkpoint_epoch_%d.pth' % (iter_epoch))
            with torch.no_grad():
                noise = torch.randn(4, config.Z_DIM, 1, 1).to(config.DEVICE)
                fakes = gen(noise, alpha, step) * 0.5 + 0.5
                print(fakes.shape)
                # fixed_fakes = make_grid(fixed_fakes, nrow=4, scale_each=True,
                #                         padding=int(0.5 * (2 ** (step+1))))
                # save_image(fixed_fakes, output_dir + 'step_{}_epoch_{}.jpg'.format(step, iter_epoch))
                fakes = make_grid(fakes, nrow=4, scale_each=True,
                                        padding=int(0.5 * (2 ** (step+1)))).permute(1, 2, 0)
                plt.imshow(fakes.cpu())
                plt.savefig(output_dir + 'step_{}_epoch_{}'.format(step, iter_epoch))
